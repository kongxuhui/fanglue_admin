<template>
    <div class="main">
        ...
        <el-tabs v-model="activeName">
            <el-tab-pane label="未处理" name="all"></el-tab-pane>
            ...
            <el-table-column label="操作" width="100">
                <template slot-scope="scope">
                    ...
                    <template v-else>
                        ...
                        <div class="mt10" v-if="scope.row.applyStatus == 2"><el-button size="mini" type="danger" @click="handleState(scope.row, 'cancel')">撤销处理</el-button></div>
                    </template>
                </template>
            </el-table-column>
        </el-table>
        ...
        <el-dialog :visible.sync="feedbackVisible">
            <el-form :model="feedback" ref="feedbackRef" label-width="80px">
                <el-form-item label="处理状态">
                    <el-input :value="APPLY_STATE[curRow.applyStatus]" disabled></el-input>
                </el-form-item>
                <el-form-item label="反馈信息" prop="feedback" :rules="{ required: true, message: '请输入审核意见', trigger: 'blur' }">
                    <el-input type="textarea" v-model.trim="feedback.feedback"></el-input>
                </el-form-item>
            </el-form>
        ...
        <el-dialog :visible.sync="followVisible">
            <el-form :model="follow" ref="followRef" label-width="80px">
                <el-form-item label="跟进状态">
                    <el-select v-model="follow.followState">
                        <el-option :label="value" :value="key" :key="key" v-for="(value, key) in FOLLOWUP_STATE"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="跟进信息" prop="followInfo" :rules="{ required: true, message: '请输入跟进信息', trigger: 'blur' }">
                    <el-input type="textarea" v-model.trim="follow.followInfo"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
...
export default {
    ...
    methods: {
        ...
        handleClick(row, type) {
            ...
            if (type === 'feedback') {
                ...
                this.feedback.feedback = row.applyFeedback;
            } else {
                this.followVisible = true;
                this.follow.followState = row.followupStatus;
                this.follow.followInfo = row.followupMessage;
            }
        },
        handleSave(type) {
            if (type === 'feedback') {
                this.$refs.feedbackRef.validate(valid => {
                    if (valid) {
                        ...
                        this.$http.post({
                            ...
                            applyFeedback: this.feedback.feedback,
                            id: this.curRow.id,
                ...
                this.$refs.followRef.validate(valid => {
                    if (valid) {
                        this.followVisible = false;
                        this.$http.post({
                            transCode: 'TCSA01',
                            funType: 'update',
                            followupStatus: this.follow.followState,
                            followupMessage: this.follow.followInfo,
                            id: this.curRow.id,
        ...
        fetchData() {
            this.getData({
                ...
                applyStatus: this.activeName === 'all' ? 1 : '',
                sourceType: this.sourceType
            })
        }
    },
    created() {
        ...
        this.activeName = this.$route.query.name || 'all';
        this.fetchData();
    },
    watch: {
        activeName() {
